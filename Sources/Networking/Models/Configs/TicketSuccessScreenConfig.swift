//
//  TicketSuccessScreenConfig.swift
//  Desk360
//
//  Created by samet on 5.11.2019.
//

public struct TicketSuccessScreenConfigModel {
...
extension TicketSuccessScreenConfigModel: Codable {
	...
	public init(from decoder: Decoder) throws {
		...
		do {
			...
			if let iconHexValue = try container.decodeIfPresent(String.self, forKey: .icon_color) {
				iconColor = UIColor.init(hex: iconHexValue)
			}
			iconSize = try (container.decodeIfPresent(Int.self, forKey: .icon_size))
	...
	public func encode(to encoder: Encoder) throws {
		...
		do {
			...
			try container.encodeIfPresent(iconSize, forKey: .icon_size)
			try container.encodeIfPresent(iconColor?.hexString(includeAlpha: true), forKey: .icon_color)
