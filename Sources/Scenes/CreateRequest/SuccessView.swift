...
final class SuccessView: UIView, Layoutable {
	...
	internal lazy var imageView: UIImageView = {
		...
		view.image = Desk360.Config.Requests.Create.successIcon
	...
	internal lazy var descriptionLabel: UILabel = {
		...
		label.textColor = Desk360.Config.currentTheme.listingPlaceholderTextColor
		label.text = "Mesajınızı aldık. En kısa sürede bir uzman mesajınıza yanıt gönderecektir."
		label.setContentCompressionResistancePriority(.required, for: .vertical)
		return label
	}()
...
//		button.setTitle("Mesajlar Sayfasına Git", for: .normal)
	...
	internal lazy var bottomDescriptionLabel: UILabel = {
		...
		label.textColor = Desk360.Config.currentTheme.listingPlaceholderTextColor
		label.text = "Bilgilendirme: Aynı koby ile ilgili birden fazla mesaj göndermemeye dikkat ediniz. Aynı konu ile ilgili birden fazla mesaj iletmeniz karışıklığa ve daha geç yanıt almanıza neden olabilir."
		label.setContentCompressionResistancePriority(.required, for: .vertical)
		return label
	}()
...
extension SuccessView {
	...
	func configure() {
		...
		print("ImageView width: \(imageView.frame.size.width)")
	...
	func configureButton() {
		...
		if imageIshidden {
			showListButton.setImage(Desk360.Config.Requests.Listing.Cell.Unread.icon, for: .normal)
