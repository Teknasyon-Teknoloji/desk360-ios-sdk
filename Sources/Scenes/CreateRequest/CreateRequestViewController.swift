...
extension CreateRequestViewController {
	func configLayoutableView() {
        if let mail = Stores.userMail.object {
            layoutableView.emailTextField.text = mail
        }
        
        if let userName = Stores.userName.object {
            layoutableView.nameTextField.text = userName
        }
